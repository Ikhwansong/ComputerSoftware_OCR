...
parser.add_argument('--data-dir', default='./data/custom')
...
parser.add_argument('--checkpoint-path', default='./checkpoints/ssd_epoch_1000.h5')
...
if __name__ == '__main__':
    ...
    batch_generator, info = create_batch_generator_with_customset(
    ...
    os.makedirs('./find_character_results/images', exist_ok=True)
    os.makedirs('./find_character_results/detects', exist_ok=True)
    visualizer = ImageVisualizer(info['idx_to_name'], save_dir='./find_character_results/images')
    ...
    for i, (filename, imgs) in enumerate(
        ...
        original_image = Image.open(
            os.path.join(info['image_dir'], '{}.jpeg'.format(filename)))
        boxes *= original_image.size * 2
        
        
        visualizer.save_image(
            original_image, boxes, classes, '{}.jpeg'.format(filename))
        
        log_file = os.path.join('./find_character_results/detects', '{}.txt')
